...
local available = {}
local unaffected = {}
local FOOD_BUFF = "food"
local FLASK_BUFF = "flask"
local foodBuffIds = {
    57286,
    19705, 24799, 33263, 44106, 57399,
    87545, 104280, 160598, 225599, 257413,
    308525, 327706, 396091, 457284,
}
local flaskBuffIds = {
    17626, 17627, 17628, 17629,
    28518, 28519, 28520, 28521, 28540, 28541,
    53752, 53755, 53758, 53760, 53763,
    79469, 79470, 79471, 79472, 79473,
    105689, 105691, 105693, 105694, 105696,
    156064, 156070, 156071, 156073, 156077, 156079, 156080, 156084,
    188031, 188033, 188034, 188035,
    251836, 251837, 251838, 251839,
    307185, 307186, 307187, 307188,
    371172, 371204, 371339, 371354, 371356, 371386,
    431971, 431972, 431973, 431974, 431975, 431976,
}
...
do
    local function AddConsumableBuff(buffKey, tag, icon, order, spellIds)
        local names = {}
        local nameSet = {}

        for _, spellId in ipairs(spellIds) do
            local name, spellIcon = F.GetSpellInfo(spellId)
        ...
        for _, required in pairs(requiredBuffs) do
            required[buffKey] = true
        end
    end

    AddConsumableBuff(FOOD_BUFF, GetLocale() == "zhCN" and "食物" or (L["Food"] or "Food"), select(2, F.GetSpellInfo(57286)) or 134062, 98, foodBuffIds)
    AddConsumableBuff(FLASK_BUFF, GetLocale() == "zhCN" and "合剂" or (L["Flask"] or "Flask"), 134821, 99, flaskBuffIds)
end
...
function F.GetUnaffectedString(spell)
    ...
    local buff = buffs[spell]["tag"] or buffs[spell]["buff1"]["name"]
...
do
    for _, k in ipairs(buffOrder) do
        tinsert(fakeIcons, CreateFakeIcon(buffs[k]["icon"] or buffs[k]["buff1"]["icon"]))
    end
end
    ...
    function b:SetTooltips(list)
        b:SetScript("OnEnter", function()
            if F.Getn(list) ~= 0 then
                ...
                CellTooltip:AddLine(L["Unaffected"] .. " |cffb7b7b7" .. (buffs[index]["tag"] or buffs[index]["buff1"]["name"]))
                for unit in pairs(list) do
...
do
    for _, k in ipairs(buffOrder) do
        buttons[k] = CreateBuffButton(buffTrackerFrame, {32, 32}, buffs[k]["buff1"] and buffs[k]["buff1"]["name"], buffs[k]["buff2"] and buffs[k]["buff2"]["name"], buffs[k]["icon"] or buffs[k]["buff1"]["icon"], k)
        buttons[k]:Hide()
...
local function UnitBuffExists(unit, buff)
    if buffs[buff]["names"] then
        local aura
        for _, name in ipairs(buffs[buff]["names"]) do
...
local function IterateAllUnits()
    ...
    if buffs[FOOD_BUFF] then
        available[FOOD_BUFF] = true
    end
    if buffs[FLASK_BUFF] then
        available[FLASK_BUFF] = true
    end
