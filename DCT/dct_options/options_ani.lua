...
local function DCT_Opani_RoundPosition(val)
	if val >= 0 then
...
local function DCT_Opani_GetFrameBoxID(obj)
	if not obj then return nil;end
	return tonumber(string.match(obj:GetName(), "(%d+)$"))
end

local function DCT_Opani_UpdatePositionSliders(fId)
	if fId ~= DCT_Opani_CurFrameID then return;end

	local p = DCT_Get("DCT_FRAME_CONFIG")[fId]
	if DCTAniOptionSlider_x then
		DCTAniOptionSlider_x:SetValue(p.x)
	end
	if DCTAniOptionSlider_y then
		DCTAniOptionSlider_y:SetValue(p.y)
	end
end

local function DCT_Opani_SaveFrameBoxPosition(obj, fId)
	local cx,cy = obj:GetCenter()
	local pcx,pcy = UIParent:GetCenter()
	if not cx or not cy or not pcx or not pcy then return;end

	local scale = obj:GetEffectiveScale() / UIParent:GetEffectiveScale()
	local p = DCT_Get("DCT_FRAME_CONFIG")[fId]
	p.x = DCT_Opani_RoundPosition(cx * scale - pcx)
	p.y = DCT_Opani_RoundPosition(cy * scale - pcy)

	obj:ClearAllPoints()
	obj:SetPoint("CENTER", UIParent, "CENTER", p.x, p.y)
	DCT_Ani_FramePos(fId, p.x, p.y)
	DCT_Opani_UpdatePositionSliders(fId)
end

local function DCT_Opani_AddFrameLabel(labels, key, cfg, saveP, fId, maxCount)
	if table.getn(labels) >= maxCount then return;end
	if not cfg or not saveP or not saveP[key] then return;end
	if saveP[key].frame ~= fId then return;end
...
local function DCT_Opani_CollectFrameLabels(labels, optionCfg, saveP, fId, maxCount)
	if not optionCfg or not optionCfg.EventFrames then return;end

	local key,cfg
	for key,cfg in pairs(optionCfg.EventFrames) do
		DCT_Opani_AddFrameLabel(labels, key, cfg, saveP, fId, maxCount)
		if table.getn(labels) >= maxCount then return;end
	end
end

local function DCT_Opani_GetFrameUsage(fId, maxCount)
	local labels = {}
	maxCount = maxCount or 2

	DCT_Opani_CollectFrameLabels(labels, DCT_OptionCfg_Event, DCT_Player, fId, maxCount)
	DCT_Opani_CollectFrameLabels(labels, DCT_OptionCfg_Event2, DCT_Player, fId, maxCount)
	DCT_Opani_CollectFrameLabels(labels, DCT_SpecCfg_Event, DCT_Player, fId, maxCount)
	DCT_Opani_CollectFrameLabels(labels, DCT_SpecCfg_Event2, DCT_Player, fId, maxCount)
	DCT_Opani_CollectFrameLabels(labels, DCTD_Config_Event, DCTD_DB, fId, maxCount)
	DCT_Opani_CollectFrameLabels(labels, DCT_SaCfg_Event, DCTSA_DB, fId, maxCount)
...
local function DCT_Opani_GetFrameBaseLabel(fId)
	return DCT_TEXT_OPANI_FRAME.." "..fId
end

local function DCT_Opani_GetFrameDropDownLabel(fId)
	local usage = DCT_Opani_GetFrameUsage(fId, 3)
	if string.len(usage) > 0 then
		return DCT_Opani_GetFrameBaseLabel(fId).." - "..usage
	end

	return DCT_Opani_GetFrameBaseLabel(fId)
end

local function DCT_Opani_GetFrameBoxUsage(fId)
	local usage = DCT_Opani_GetFrameUsage(fId, 1)
	local aniType = DCT_Opani_AniTypeConfig[DCT_Get("DCT_FRAME_CONFIG")[fId].aniType]

	if string.len(usage) > 0 and aniType then
		return usage.."/"..aniType
	elseif string.len(usage) > 0 then
		return usage
	end
...
function DCT_Opani_FrameBoxRefresh()
	...
	for i = 1,DCT_Ani_MaxFrame do
		...
		if tp[i].active then
			...
			getglobal("DCTAniOptionFrameBox"..i.."_Title1"):SetText(DCT_Opani_GetFrameBaseLabel(i))
			getglobal("DCTAniOptionFrameBox"..i.."_Title2"):SetText(DCT_Opani_GetFrameBoxUsage(i))
			obj:Show()
...
function DCT_Opani_FrameDDC_Initialize()
	...
	for i = 1, DCT_Ani_MaxFrame do
		info = {
			text = DCT_Opani_GetFrameDropDownLabel(i);
...
function DCT_Opani_FrameDDC_OnShow(self)
	...
	UIDropDownMenu_SetText(self, DCT_Opani_GetFrameDropDownLabel(DCT_Opani_CurFrameID))
...
function DCT_Opani_FrameDDC_OnClick(self)
	...
	UIDropDownMenu_SetText(DCTAniOptionFrameDDC, DCT_Opani_GetFrameDropDownLabel(thisID))
...
function DCTFrameBoxMouseDown(self, button)
	local fId = DCT_Opani_GetFrameBoxID(self)
...
function DCTFrameBoxMouseUp(self, button)
	...
	local fId = DCT_Opani_GetFrameBoxID(self)
	if not fId then return;end
	DCT_Opani_SaveFrameBoxPosition(self, fId)
	DCT_Opani_FrameBoxRefresh()
end
